...
FOLDER = r"C:\zolpo\keshettaamim\keshettaamim_prices_stores_auto"
            ...
            for subchain in subchains_root.findall('SubChain'):
                ...
                if stores is not None:
                    for store in stores.findall('Store'):
                        store_id_elem = store.find('StoreID')       # שים לב: ID גדול
                        store_name_elem = store.find('StoreName')   # שים לב: Name גדול
...
schedule.every().day.at("11:20").do(convert_all_xml_to_json)

if __name__ == "__main__":
    print("⏳ ממתין להפעלה יומית ב־15:40...")
    while True:
        try:
