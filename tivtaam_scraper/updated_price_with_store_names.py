...
def load_store_mapping():
    mapping_file = r"C:\Zolpo\tivtaam\tivtaam_prices_stores_auto\flat_store_mapping_tivtaam.json"
    try:
        with open(mapping_file, 'r', encoding='utf-8') as f:
            store_mapping = json.load(f)
        print(f"📦 נטען מיפוי של {len(store_mapping)} סניפים מקובץ flat_store_mapping_tivtaam.json")
...
def update_price_files():
    ...
    input_folder = r"C:\Zolpo\tivtaam\tivtaamprices_auto_json"
    output_folder = r"C:\Zolpo\tivtaam\tivtaamprices_auto_json_updated"
    report_folder = r"C:\Zolpo\tivtaam\reports"
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith(".json") and ("Price" in filename or "PriceFull" in filename):
            ...
            try:
                ...
                # תמיכה גם ב-Root וגם ב-root
                root = price_data.get("Root") or price_data.get("root", {})
                ...
                updated_root = {key: root[key] for key in ["StoreId", "StoreName", "ChainId", "SubChainId", "BikoretNo", "DllVerNo"] if key in root}
                updated_root["Items"] = root.get("Items", {})

                # שימור שם המפתח המקורי בקובץ
                if "Root" in price_data:
                    price_data["Root"] = updated_root
                else:
                    price_data["root"] = updated_root
...
# תזמון יומי
schedule.every().day.at("10:31").do(update_price_files)
