...
FOLDER = r"C:\zolpo\salachdabach\salachdabach_prices_stores_auto"
...
def parse_xml(file_path):
    try:
        ...
        # Root > SubChains > SubChain > Stores > Store
            ...
            for subchain in subchains_root.findall('SubChain'):
                ...
                if stores is not None:
                    for store in stores.findall('Store'):
                        store_id_elem = store.find('StoreId')  # ← תיקון חשוב כאן
        ...
        # שמירת JSON רק אם נמצא מידע
        json_file_path = file_path.replace('.xml', '.json')
        with open(json_file_path, 'w', encoding='utf-8') as json_file:
            json.dump(store_data, json_file, ensure_ascii=False, indent=4)

        if store_data:
            print(f"✅ המרת {os.path.basename(file_path)} ל-JSON הצליחה ({len(store_data)} סניפים)")
        else:
            print(f"⚠️ קובץ {os.path.basename(file_path)} לא הכיל סניפים")

        return json_file_path

    except Exception as e:
        print(f"❌ שגיאה בהמרת {file_path}: {e}")
        return None
...
# תזמון יומי
schedule.every().day.at("11:02").do(convert_all_xml_to_json)
